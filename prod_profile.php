    ...
    public function index(Request $request)
    {
        ...
        if ($user) {
            ...
            if ($member) {
                ...
                $allMembers = Member::withSum(['results as monthly_event_points' => function($query) use ($currentMonth, $currentYear) {
                        $query->whereMonth('result_date', $currentMonth)
                              ->whereYear('result_date', $currentYear);
                    }], 'event_points')
                    ->withSum(['results as monthly_placement_bonus' => function($query) use ($currentMonth, $currentYear) {
                        $query->whereMonth('result_date', $currentMonth)
                              ->whereYear('result_date', $currentYear);
                    }], 'placement_bonus')
                    ->get()
